...
export default function AllUsersReport() {
  ...
  return (
    <section className="flex w-full flex-col border-y border-ink/10 bg-paper/80 py-8">
      ...
      <h1 className="font-display text-[clamp(1.5rem,2.4vw,2rem)] font-semibold tracking-tight text-ink-deep mt-2 text-4xl">All Users Report</h1>
      ...
      {isLoading ? (
        <div className="mt-8 font-medium text-ink/65">Loading all users report...</div>
      ) : (
        <>
          <div className="mt-8">
            <p className="text-sm font-semibold text-ink/70">{dateRangeLabel}</p>
          </div>
              ...
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  {filteredUsers.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-4 py-5 text-center font-medium text-ink/50">
                    ...
                    filteredUsers.map((user) => (
                      ...
                      >
                        <td className="px-4 py-3 font-medium text-ink-deep">{user.userId}</td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-semibold ${NormalizeAccountStatus(user.accountStatus) === 'Active' ? 'bg-success/10 text-success ring-1 ring-inset ring-success/25' : 'bg-danger/10 text-danger ring-1 ring-inset ring-rose-600/20'}`}>
